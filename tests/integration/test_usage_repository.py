...
@pytest.mark.asyncio
async def test_latest_by_account_sqlite_account_filter_is_scoped(db_setup):
    now = utcnow()
    async with SessionLocal() as session:
        if _dialect_name(session) != "sqlite":
            pytest.skip("SQLite-only account-scope test")
        ...
        await repo.add_entry("acc2", 20.0, window="primary", recorded_at=now)

        scoped = await repo.latest_by_account(window="primary", account_ids=["acc1"])

    assert set(scoped) == {"acc1"}
